...
到 Settings > General 使用 CLI 安裝功能，為 AppImage 建立 `~/.local/bin/snaptray` wrapper。請確認 `~/.local/bin` 已在你的 `PATH` 中，安裝或移除後請重新開啟終端機。
...
| `record` | 開始、停止或切換錄影（僅 macOS/Windows） | 是 |
| `pin` | 釘選圖片檔或剪貼簿圖片 | 是 |
...
snaptray record start                 # 開始錄影（僅 macOS/Windows）
snaptray record stop                  # 停止錄影（僅 macOS/Windows）
snaptray record                       # 切換錄影（僅 macOS/Windows）
snaptray record start -n 1            # 在螢幕 1 開始全螢幕錄影（僅 macOS/Windows）
...
- `record` 在 macOS 與 Windows 只接受 `start`、`stop`、`toggle`。不給 action 時預設是 `toggle`。目前實作中，`-n/--screen` 只會被 `record start` 使用。
- Linux beta 不包含錄影。`record` 指令不會出現在 help 輸出；若直接呼叫會回傳錄影錯誤。
...
| `3` | 檔案錯誤 |
| `4` | 實例錯誤（主程式未執行） |
| `5` | 錄影錯誤，包含 Linux beta 直接呼叫 `record` |
