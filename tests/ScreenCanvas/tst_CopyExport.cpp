...
private slots:
    ...
    void testScreenSnapshotAlreadyHiddenPathDoesNotRestorePrematurely();
};
...
void TestScreenCanvasCopyExport::testScreenSnapshotAlreadyHiddenPathDoesNotRestorePrematurely()
{
    ScreenCanvasSession session;
    session.m_qmlToolbar.reset();
    session.m_qmlSubToolbar.reset();
    ...
    bool snapshotSawHiddenSurface = false;
    session.m_screenSnapshotProvider = [&](QScreen* targetScreen) {
        snapshotSawHiddenSurface = (targetScreen == screen) && !surface.isVisible();
    ...
    const ScreenCanvasSession::ScreenSnapshotVisibilityState hiddenUiState =
        session.hideUiForScreenSnapshot();
    QVERIFY(!surface.isVisible());

    const QPixmap snapshot = session.buildCopyExportBasePixmap(screen, true);

    QVERIFY(!snapshot.isNull());
    QVERIFY(snapshotSawHiddenSurface);
    QVERIFY(!surface.isVisible());

    session.restoreUiAfterScreenSnapshot(hiddenUiState);
    QCoreApplication::processEvents();
    QVERIFY(surface.isVisible());
