...
void ScrollingCaptureManager::startCapturing()
{
    ...
    // Create global ESC shortcut to stop capture
    // Use highlighter as parent so it gets keyboard focus
    if (m_highlighter && !m_escShortcut) {
        m_escShortcut = new QShortcut(Qt::Key_Escape, m_highlighter);
        m_escShortcut->setContext(Qt::ApplicationShortcut);
        connect(m_escShortcut, &QShortcut::activated,
                this, &ScrollingCaptureManager::onStopClicked);
    }
