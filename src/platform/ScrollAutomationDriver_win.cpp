...
constexpr int kSyntheticWheelNotchesPerStep = 2;
constexpr int kSyntheticEstimatedDeltaY = 160;
...
public:
    ...
    ScrollStepResult step() override
    {
        ...
        if (m_mode == DriverMode::SyntheticWheel) {
            ...
            const bool shouldRefocus = m_syntheticStepsSinceFocus >= 12;
            if (shouldRefocus) {
            ...
            if (m_targetWindow) {
                injected = injectWheelStepToWindow(
                    m_targetWindow,
                    m_anchorPoint,
                    static_cast<short>(-WHEEL_DELTA * kSyntheticWheelNotchesPerStep));
                targetLocked = injected;
            }
            if (!injected) {
                injected = injectWheelStep(-WHEEL_DELTA * kSyntheticWheelNotchesPerStep);
            }
        ...
        hr = scrollPattern->Scroll(ScrollAmount_NoAmount, ScrollAmount_SmallIncrement);
        if (FAILED(hr)) {
            scrollPattern->Release();
        ...
        result.estimatedDeltaYConfidence = 0.65;
