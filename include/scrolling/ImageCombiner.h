...
/**
 * @brief Result of template matching for overlap detection
 */
struct TemplateMatchResult {
    int overlapLines;       // Number of overlapping rows
    double confidence;      // Match confidence (0-1)
    bool valid;             // Whether match is reliable
};
...
private:
    ...
    /**
     * @brief Find overlap using OpenCV template matching
     *
     * More robust than line-by-line comparison for apps with similar content.
     * Uses bottom portion of result as template, searches in top of current.
     *
     * @param result Accumulated result image
     * @param current Current frame
     * @param ignoreTop Rows to skip at top (sticky header)
     * @param ignoreBottom Rows to skip at bottom (sticky footer)
     * @param sideMargin Pixels to ignore on sides (scrollbar)
     * @return Match result with overlap lines and confidence
     */
    TemplateMatchResult findOverlapWithTemplate(
