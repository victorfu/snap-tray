...
layout: docs
title: CLI
seo_title: "SnapTray CLI: 스크린샷 및 플랫폼별 녹화 자동화"
description: 캡처, 플랫폼별 녹화, 핀, 설정 워크플로를 스크립팅하기 위한 공식 자동화 인터페이스.
...
| `record` | 녹화 시작, 중지 또는 전환 (macOS/Windows 전용) | 예 |
...
snaptray record start                 # 녹화 시작 (macOS/Windows 전용)
snaptray record stop                  # 녹화 중지 (macOS/Windows 전용)
snaptray record                       # 녹화 전환 (macOS/Windows 전용)
snaptray record start -n 1            # 화면 1에서 전체 화면 녹화 시작 (macOS/Windows 전용)
...
- `record`는 macOS 및 Windows에서 `start`, `stop`, 또는 `toggle`을 인수로 받습니다. 인수 없으면 `toggle`입니다. 현재 구현에서 `-n/--screen`은 `record start`에서만 처리됩니다.
- Linux beta는 녹화를 포함하지 않습니다. `record` 명령은 도움말 출력에서 숨겨져 있으며, 직접 호출하면 녹화 오류를 반환합니다.
...
| `5` | 녹화 오류 (Linux beta에서 `record` 직접 호출 포함) |
