...
- **設定對話框**：
  - General 分頁：開機自動啟動、顯示語言、App 主題（深色/淺色）、macOS 權限快捷入口、CLI 安裝/移除
  - Hotkeys 分頁：依分類管理熱鍵（Capture/Canvas/Clipboard/Pin/Recording）
  - Advanced 分頁：截圖快捷提示、Auto Blur 與釘選視窗預設值（透明度/縮放/快取）
  ...
  - Updates 分頁：目前版本、自動檢查、檢查頻率、上次檢查狀態、立即檢查
  - About 分頁：App 資訊、版本、作者與網站
  - 背景會定期自動檢查更新
  - 設定儲存於系統設定 (QSettings)
...
|   |-- qml/               # QML view model、overlay 包裝與橋接層
|   |-- region/            # 區域選取元件
|   |-- settings/          # 設定管理器
|   |-- share/             # 分享上傳客戶端
|   |-- toolbar/           # 工具列繪製
...
|   |-- qml/               # Qt Quick overlay、對話框、工具列與設定頁
|   |   |-- components/    # 共用 QML 視覺元件
|   |   |-- controls/      # 可重用 QML 控制項
|   |   |-- dialogs/       # 結果與分享對話框
|   |   |-- panels/        # 截圖 overlay 面板（region control、多區域列表）
|   |   |-- recording/     # 錄影 overlay 與預覽 UI
|   |   |-- settings/      # 設定視窗頁面
|   |   |-- toolbar/       # 浮動工具列 UI
|   |   `-- tokens/        # QML 設計 token
|   |-- region/            # 區域選取實作
...
| `QmlOverlayManager`         | `src/qml/`         | 共用 QQmlEngine 與浮動 overlay 視窗建立  |
| `QmlOverlayPanel`           | `src/qml/`         | 通用浮動 QML overlay 包裝器              |
| `RegionControlViewModel`    | `src/qml/`         | 圓角半徑/比例鎖控制的狀態與動作橋接 |
| `RegionControlPanel`        | `src/qml/panels/`  | 區域控制 overlay UI         |
| `MultiRegionListViewModel`  | `src/qml/`         | 多區域資料與縮圖橋接       |
| `MultiRegionListPanel`      | `src/qml/panels/`  | 多區域列表 UI 面板         |
