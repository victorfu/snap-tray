...
void QRCodeResultDialog::setPinActionAvailable(bool available)
{
    m_pinButton->setVisible(available);
    if (!available) {
        m_pinButton->setEnabled(false);
    }
}
...
void QRCodeResultDialog::onGenerateClicked()
{
    ...
    if (!QRCodeManager::canEncode(text, kGenerateEccLevel)) {
        return;
    }
...
void QRCodeResultDialog::onTextChanged()
{
    ...
    // Generated preview is tied to the exact text at generation time.
    // Any edit invalidates it and requires explicit regeneration.
...
{
    const QString text = currentText();
    const bool canGenerate = QRCodeManager::canEncode(text, kGenerateEccLevel);
    ...
    if (!canGenerate) {
        ...
        if (text.isEmpty()) {
            ...
            m_generatedPreviewLabel->setText(
                tr("Content too long (max %1 bytes)")
                    .arg(QRCodeManager::maxEncodableLength(kGenerateEccLevel)));
        }
        return;
    }
