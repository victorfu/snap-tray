...
- **相依套件**（以 FetchContent 自動取得）:
  ...
  - [OpenCV 4.10.0](https://github.com/opencv/opencv)（臉部/文字偵測管線 + `opencv_photo` 接縫融合）
  - [OpenCV contrib 4.10.0](https://github.com/opencv/opencv_contrib)（`xfeatures2d` 的 SURF 捲動對齊，需 `OPENCV_ENABLE_NONFREE=ON`）
  - [hnswlib v0.8.0](https://github.com/nmslib/hnswlib)（捲動特徵後備路徑的近鄰搜尋）
  ...
  - 注意：啟用 OpenCV nonfree 模組後，發佈前需進行額外授權/合規檢查。
- **macOS 原生框架**:
